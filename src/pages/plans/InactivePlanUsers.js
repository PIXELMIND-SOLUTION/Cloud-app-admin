// pages/InactivePlanUsers.js - Light Theme with Saffron Accents
...
export const InactivePlanUsers = () => {
    ...
    const UserCard = ({ user }) => {
        ...
        return (
            <div
                className="rounded-xl p-4 transition-all duration-200 opacity-75 hover:opacity-100 cursor-pointer bg-white border border-gray-200 shadow-sm hover:border-orange-200 hover:shadow-md"
                onClick={() => handleUserClick(user)}
            >
                <div className="flex items-start gap-3">
                    <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${user.avatarGrad || 'from-gray-400 to-gray-500'} flex items-center justify-center text-white text-sm font-bold shrink-0 grayscale`}>
                        {user.avatar || user.name?.charAt(0)}
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between">
                            <div>
                                <p className="text-sm font-semibold truncate text-gray-600">{user.name}</p>
                                <p className="text-xs truncate flex items-center gap-1 text-gray-500">
                                    <Mail size={10} /> {user.email}
                                </p>
                            </div>
                            <span className="flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full whitespace-nowrap bg-gray-100 text-gray-500">
        ...
        <div className="space-y-6 bg-gray-50 min-h-screen p-4 rounded-2xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-orange-200 shadow-sm">
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <div>
                        <h1 className="text-lg sm:text-xl font-bold text-gray-800">Inactive Users</h1>
            ...
            <Panel className="p-4">
                <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <select
                        ...
                        className="px-3.5 py-2.5 text-sm rounded-xl transition-all bg-orange-50 border border-orange-200 text-gray-800 focus:border-orange-400 focus:ring-2 focus:ring-orange-200"
                    >
                        <option value="lastSeen" className="bg-white">Last Seen</option>
            ...
            {sortedUsers.length === 0 ? (
                <div className="text-center py-16 rounded-2xl bg-white border border-orange-200">
                    ...
                    <p className="text-sm font-medium text-gray-800">No inactive users found</p>
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {paginatedUsers.map(user => (
                                    <tr key={user.id} className="border-b border-orange-100 hover:bg-orange-50 transition-colors opacity-75 hover:opacity-100">
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${user.avatarGrad || 'from-gray-400 to-gray-500'} flex items-center justify-center text-white text-xs font-bold grayscale`}>
                                                    {user.avatar || user.name?.charAt(0)}
                                                </div>
                                                <div>
                                        ...
                                        <td className="px-4 py-4 text-sm text-gray-600">{user.role || 'User'}</td>
                                        <td className="px-4 py-4 text-sm text-gray-600">{user.totalDevices || 0}</td>
                                        ...
                                        <td className="px-4 py-4 text-xs text-gray-500">{user.lastSeen || 'Unknown'}</td>
                                        <td className="px-4 py-4">
