// pages/AdminPlanManagement.js - Optimized & Responsive
...
const StatCard = ({ label, value, icon: Icon, grad }) => (
    <div className="rounded-2xl p-4 flex items-center gap-3 transition-all hover:border-purple-500/30"
        style={{
            background: 'rgba(20, 16, 36, 0.8)',
            border: '1px solid rgba(139,92,246,0.15)',
            backdropFilter: 'blur(12px)',
    ...
    >
        ...
        <div className="min-w-0">
            <p className="text-[10px] font-medium truncate" style={{ color: '#5a4f72' }}>{label}</p>
            <p className="text-lg sm:text-xl font-bold truncate" style={{ color: '#e2d9f3' }}>{value}</p>
        </div>
    </div>
);
...
const PlanFormModal = ({ isOpen, onClose, plan, onSave }) => {
    ...
    const Field = ({ label, error, children }) => (
        <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: '#9c8fc0' }}>{label}</label>
            {children}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <div className="rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col"
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'rgba(139,92,246,0.15)' }}>
                    <h2 className="text-lg font-bold" style={{ color: '#e2d9f3' }}>{plan ? 'Edit Plan' : 'Create New Plan'}</h2>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <Field label="Plan Name *" error={errors.name}>
                            <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })}
                                ...
                                style={{ background: 'rgba(139,92,246,0.08)', border: errors.name ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }} />
                        </Field>
                        ...
                        <Field label="Description *" error={errors.description}>
                            <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })}
                                ...
                                style={{ background: 'rgba(139,92,246,0.08)', border: errors.description ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }} />
                        </Field>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <Field label="Price ($) *" error={errors.price}>
                                <input type="number" min="0" step="0.01" value={formData.price}
                                    ...
                                    style={{ background: 'rgba(139,92,246,0.08)', border: errors.price ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }} />
                            </Field>
                            <Field label="Discount Type">
                                <select value={formData.discountType} onChange={e => setFormData({ ...formData, discountType: e.target.value })}
                                    ...
                                    <option value="percentage" style={{ background: '#1a1430' }}>Percentage (%)</option>
                                    <option value="fixed" style={{ background: '#1a1430' }}>Fixed ($)</option>
                                </select>
                            </Field>
                            <Field label="Discount Amount" error={errors.discountAmount}>
                                <input type="number" min="0" step={formData.discountType === 'percentage' ? 1 : 0.01}
                                    ...
                                    className="w-full px-4 py-2.5 rounded-xl transition-all"
                                    style={{ background: 'rgba(139,92,246,0.08)', border: errors.discountAmount ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }} />
                            </Field>
                        </div>
                        ...
                        <Field label="Features *" error={errors.features}>
                            <div className="space-y-2">
                                {formData.features.map((feature, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        <input type="text" value={feature} onChange={e => updateFeature(index, e.target.value)}
                                            ...
                                            style={{ background: 'rgba(139,92,246,0.08)', border: errors.features ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }} />
                                ...
                                <button type="button" onClick={addFeature} className="flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-purple-300" style={{ color: '#a78bfa' }}>
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'rgba(139,92,246,0.15)' }}>
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs" style={{ color: '#5a4f72' }}>Created {new Date(plan.createdAt).toLocaleDateString()}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-1.5 rounded-lg transition-colors hover:bg-purple-500/10" style={{ color: '#5a4f72' }}>
                        <X size={20} />
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="rounded-xl p-4" style={{ background: 'rgba(139,92,246,0.08)' }}>
                            ...
                            <div className="flex items-baseline gap-2 mt-1 flex-wrap">
                                ...
                                {plan.discountAmount > 0 && <span className="text-sm line-through" style={{ color: '#5a4f72' }}>${plan.price}</span>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="rounded-xl p-4" style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.1)' }}>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <div className="text-center cursor-pointer rounded-lg p-2 transition-colors hover:bg-emerald-500/10"
                            ...
                            <div className="text-center cursor-pointer rounded-lg p-2 transition-colors hover:bg-slate-400/10"
                                ...
                                <p className={`text-2xl font-bold ${plan.inactiveUsers > 0 ? 'text-slate-400' : ''}`} style={{ color: plan.inactiveUsers > 0 ? '#94a3b8' : '#5a4f72' }}>
        ...
        <Panel className="p-4 space-y-3">
            <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3 min-w-0">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${plan.status === 'active' ? 'from-emerald-400 to-teal-500' : 'from-slate-400 to-slate-500'} flex items-center justify-center shrink-0`}>
                        <Package size={16} className="text-white" />
                    </div>
                    <div className="min-w-0">
                        <p className="text-sm font-semibold truncate" style={{ color: '#e2d9f3' }}>{plan.name}</p>
                        <p className="text-xs truncate" style={{ color: '#5a4f72' }}>{plan.description}</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-xl py-2" style={{ background: 'rgba(139,92,246,0.06)' }}>
                    <p className="text-sm font-bold" style={{ color: '#e2d9f3' }}>${plan.price}</p>
                    <p className="text-[10px]" style={{ color: '#5a4f72' }}>Price</p>
                </div>
                <div className="rounded-xl py-2" style={{ background: 'rgba(139,92,246,0.06)' }}>
                    <p className="text-sm font-bold" style={{ color: '#e2d9f3' }}>{plan.features.length}</p>
                    <p className="text-[10px]" style={{ color: '#5a4f72' }}>Features</p>
                </div>
                <div className="rounded-xl py-2" style={{ background: 'rgba(139,92,246,0.06)' }}>
                    <p className="text-sm font-bold" style={{ color: '#e2d9f3' }}>{plan.subscriberCount || 0}</p>
                    <p className="text-[10px]" style={{ color: '#5a4f72' }}>Users</p>
                </div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
                <button onClick={() => onViewActive(plan)} className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium transition-colors ${plan.activeUsers > 0 ? 'hover:bg-emerald-500/20' : 'opacity-50 cursor-not-allowed'}`}
                ...
                <button onClick={() => setExpanded(!expanded)} className="ml-auto p-1 rounded-lg transition-colors hover:bg-purple-500/10" style={{ color: '#5a4f72' }}>
                    {expanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </button>
            </div>

            {expanded && (
                <div className="pt-3 space-y-2" style={{ borderTop: '1px solid rgba(139,92,246,0.1)' }}>
                    <div className="flex items-center justify-between text-xs" style={{ color: '#5a4f72' }}>
                        <span>Updated: {new Date(plan.updatedAt).toLocaleDateString()}</span>
                        <span>ID: {plan.id}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                        <button onClick={() => onView(plan)} className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg transition-colors hover:bg-purple-500/10" style={{ color: '#a78bfa' }}>
                            <Eye size={14} /> View
                        </button>
                        <button onClick={() => onEdit(plan)} className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg transition-colors hover:bg-blue-500/10" style={{ color: '#60a5fa' }}>
                            <Edit size={14} /> Edit
                        </button>
                        <button onClick={() => onToggleStatus(plan.id)} className={`flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg transition-colors ${plan.status === 'active' ? 'hover:bg-amber-500/10' : 'hover:bg-emerald-500/10'}`}
                            style={{ color: plan.status === 'active' ? '#fcd34d' : '#34d399' }}>
                            {plan.status === 'active' ? 'Deactivate' : 'Activate'}
                        </button>
                        <button onClick={() => onDelete(plan.id)} className="flex items-center gap-1 px-3 py-1.5 text-xs rounded-lg transition-colors hover:bg-red-500/10 ml-auto" style={{ color: '#f87171' }}>
    ...
    return (
        <div className="space-y-4 sm:space-y-6 px-2 sm:px-0">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h1 className="text-lg sm:text-xl font-bold" style={{ color: '#e2d9f3' }}>Plan Management</h1>
                    <p className="text-xs sm:text-sm mt-0.5" style={{ color: '#5a4f72' }}>Create, manage, and customize subscription plans</p>
                </div>
                <button onClick={() => { setSelectedPlan(null); setIsFormOpen(true); }}
                    className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white rounded-xl transition-all shadow-sm hover:shadow-purple-500/50 shrink-0"
                    style={{ background: 'linear-gradient(135deg, #7c3aed, #9333ea)', boxShadow: '0 0 10px rgba(124,58,237,0.2)' }}>
            ...
            <Panel>
                <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)}
                        ...
                        style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.15)', color: '#e2d9f3' }}>
                        {['All', 'active', 'inactive'].map(s => <option key={s} value={s} style={{ background: '#1a1430' }}>{s}</option>)}
                    </select>
                    <button onClick={loadPlans} className="flex items-center justify-center gap-2 px-3.5 py-2.5 text-sm rounded-xl transition-colors hover:bg-purple-500/10"
            ...
            <div className="md:hidden space-y-3">
                {loading ? (
                    <div className="text-center py-16">
                        <div className="animate-spin rounded-full h-8 w-8 border-2 border-violet-400 border-t-transparent mx-auto" />
                        <p className="text-sm mt-4" style={{ color: '#5a4f72' }}>Loading plans...</p>
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="text-center py-16 rounded-2xl" style={{ background: 'rgba(20,16,36,0.8)', border: '1px solid rgba(139,92,246,0.15)' }}>
                        <Package size={32} className="mx-auto mb-3 opacity-30" style={{ color: '#5a4f72' }} />
            ...
            <Panel className="hidden md:block overflow-hidden p-0">
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y" style={{ borderColor: 'rgba(139,92,246,0.06)' }}>
                            {loading ? (
                                ...
                                <tr key={plan.id} className="transition-colors hover:bg-purple-500/5" style={{ borderColor: 'rgba(139,92,246,0.06)' }}>
                                    <td className="px-5 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${plan.status === 'active' ? 'from-emerald-400 to-teal-500' : 'from-slate-400 to-slate-500'} flex items-center justify-center shrink-0`}>
                                    ...
                                    <td className="px-4 py-4 text-sm font-bold" style={{ color: '#e2d9f3' }}>${plan.price}</td>
                                    ...
                                    <td className="px-4 py-4 text-sm" style={{ color: '#9c8fc0' }}>{plan.features.length}</td>
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-1.5">
                                            <button onClick={() => handleViewActiveUsers(plan)}
                                                className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium transition-colors ${plan.activeUsers > 0 ? 'hover:bg-emerald-500/20' : 'opacity-50 cursor-not-allowed'}`}
                                                style={plan.activeUsers > 0 ? { background: 'rgba(52,211,153,0.12)', color: '#34d399' } : { background: 'rgba(100,116,139,0.08)', color: '#5a4f72' }}>
                                    ...
                                    <td className="px-4 py-4">
                                        <button onClick={() => handleToggleStatus(plan.id)}
                                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-colors hover:${plan.status === 'active' ? 'bg-emerald-500/20' : 'bg-slate-400/20'}`}
                                            style={plan.status === 'active' ? { background: 'rgba(52,211,153,0.12)', color: '#34d399' } : { background: 'rgba(100,116,139,0.08)', color: '#94a3b8' }}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${plan.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                                            {plan.status}
                                        </button>
                                    </td>
                                    <td className="px-4 py-4 text-xs" style={{ color: '#5a4f72' }}>{new Date(plan.updatedAt).toLocaleDateString()}</td>
                                    <td className="px-4 py-4">
                                        <div className="flex items-center gap-1.5">
                                            <button onClick={() => { setViewingPlan(plan); setIsViewOpen(true); }}
                                                className="p-1.5 rounded-lg transition-colors hover:bg-purple-500/10" style={{ color: '#5a4f72' }}>
                                                <Eye size={16} />
                                            </button>
                                            <button onClick={() => { setSelectedPlan(plan); setIsFormOpen(true); }}
                                                className="p-1.5 rounded-lg transition-colors hover:bg-blue-500/10" style={{ color: '#5a4f72' }}>
                                                <Edit size={16} />
                                            </button>
                                            <button onClick={() => handleDelete(plan.id)}
                                                className="p-1.5 rounded-lg transition-colors hover:bg-red-500/10" style={{ color: '#5a4f72' }}>
                                                <Trash2 size={16} />
                                            </button>
                                        </div>
                                    </td>
                ...
                <div className="px-5 py-3 border-t flex items-center justify-between" style={{ borderColor: 'rgba(139,92,246,0.1)' }}>
                    <p className="text-xs" style={{ color: '#5a4f72' }}>Showing {filtered.length} of {plans.length} plans</p>
                </div>
            </Panel>
